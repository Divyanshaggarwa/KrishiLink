...
export default async function BuyerOrdersPage() {
  ...
  const { data: orders } = await supabase
    .from("transactions")
    .select(
      "id, final_price_per_kg, quantity_kg, gross_amount, logistics_cost_per_kg, distance_km, vehicle_type, status, created_at, farmer_id, listing:listings!inner(crop, quality_grade)"
    )
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-[#6B7A74]">
                      Farmer: <strong>{f?.full_name}</strong> · {f?.district},{" "}
                      {f?.state} · {o.distance_km} km · {o.vehicle_type}
                    </p>
                  </div>
