  ...
  return (
    <div className="rounded-2xl border border-[#C8E6C9] bg-[#FAFCFA] p-5">
      ...
      <p className="mt-1 text-xs text-[#6B7A74]">
        Fair midpoint: <strong className="text-[#1B4D3E]">₹{band.mid.toFixed(2)}</strong>
        {" · "}Quality ×{band.qualityMultiplier}
      </p>

      {/* Range bar with two markers */}
      <div className="mt-5">
        <div className="relative h-2 rounded-full bg-[#E4EBE6]">
          <div
            className="absolute inset-y-0 rounded-full bg-[#A5D6A7]"
      ...
      <div className="mt-5 rounded-xl border border-[#E4EBE6] bg-white p-4">
        ...
        <div className="mt-2 space-y-1.5 text-xs">
          <Row label="Transport share" value={`₹${band.transportSharePerKg.toFixed(2)}/kg`} />
          <Row label="Labour / handling" value={`₹${band.labourPerKg.toFixed(2)}/kg`} />
          <Row label="Payment / platform" value={`₹${band.txnSharePerKg.toFixed(2)}/kg`} />
